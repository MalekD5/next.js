...
Inside your `@auth` slot, intercept the `/login` route by renaming the `@auth/login` folder to `@auth/(.)login`. Then, import the `<Modal>` component and its children into the `@auth/(.)login/page.tsx` file.
