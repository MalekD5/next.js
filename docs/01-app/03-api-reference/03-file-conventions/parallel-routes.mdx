...
Inside your `@auth` slot, intercept the `/login` route by first renaming `@auth/login` folder to `@auth/(.)login`. After that, import `<Modal>` component and its children into the `@auth/(.)login/page.tsx` file.
