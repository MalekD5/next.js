...
function pingRootRouteTree(
  ...
  switch (route.status) {
    ...
    case EntryStatus.Fulfilled: {
      ...
      const fetchStrategy = getFetchStrategyForTask(task, route)
...
function getFetchStrategyForTask(
  task: PrefetchTask,
  route: RouteCacheEntry
): FetchStrategy {
  const { includeDynamicData } = task
  switch (includeDynamicData) {
    case 'dynamic': {
    ...
    case null: {
      return route.isPPREnabled
        ? FetchStrategy.PPR
        : FetchStrategy.LoadingBoundary
    }
    default: {
      includeDynamicData satisfies never
      throw new InvariantError('Unknown IncludeDynamicData')
    }
  }
}
...
function isFetchStrategyLessSpecific(
  ...
  switch (newStrategy) {
    ...
    default: {
      ...
      throw new InvariantError('Unknown FetchStrategy')
    }
  }
}
